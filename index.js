...
const writeOrdersToExcelBuffer = async (orders) => {
  // Prepare order data
...
async function run() {
  try {
    ...
    app.get('/download-orders', async (req, res) => {
      try {
        const orders = await bookingProductsCollection.find().toArray();
        const buffer = await writeOrdersToExcelBuffer(orders);
